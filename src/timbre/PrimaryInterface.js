...
stage.scale.set(1/getPixelDensity());
...
export function init() {
	...
	$(document).ready(() => {
		updateSize();
		renderer = new PIXI.autoDetectRenderer(width, height, {resolution: getPixelDensity(), transparent: false, backgroundColor: 0x000000, antialiasing: true});
...
function handlePointerMove(positionX, positionY) {
	if(placing) {
		...
		const distance = mouse.distance(activeAnchor);
